...
<!--      <uni-badge text="5" type="error"></uni-badge> -->
    ...
    <scroll-view scroll-y class="order-list">
      <view v-for="order in saleorders" :key="order.id" class="order-item">
        <!-- 店铺信息 -->
        <view class="shop-header">
          <text class="shop-name">Recalue平台闲置</text>
...
import { getsaleorder, saleorderDetail } from '../../api/mine/mine.js'

const popup = ref(null)

const activeTab = ref('已完成')
const tabs = ['待发货', '待服务', '已完成', '待评价', '已取消']
...
const checkorder = async(id)=>{
	const res = await saleorderDetail(id)
	if(res.code){
...
const InitSaleorder = async()=>{
	const res = await getsaleorder(currentUserId.value)
	if(res.code){
